...
public class CommentHandler implements Runnable {
  ...
  public void run() {
    ...
    try {
      BufferedReader inputFromViewer = new BufferedReader(
  ...
  private void broadcastMessage(String message,
      Socket sock) { // send message to viewers in socketList except sock
    try {
      int i = 2;
      while (this.socketList[i] != null) {
        if (this.serverOutputToViewers[i] == null) {
          this.serverOutputToViewers[i] = new DataOutputStream(
              this.socketList[i].getOutputStream());
        }
